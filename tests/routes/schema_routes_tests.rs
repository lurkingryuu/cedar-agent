...
async fn test_schema_change_validates_policies() {
    ...
    let result = policy_store
        .update_policies(policies, Some(new_schema.clone().try_into().unwrap()))
        .await;
...
async fn test_schema_change_validates_entities() {
    ...
    let result = data_store
        .update_entities(entities, Some(new_schema.try_into().unwrap()))
        .await;
...
async fn test_schema_with_namespaces() {
    ...
    let stored = schema_store.get_internal_schema().await;
    let has_namespace = stored
        .get()
        .as_object()
        .map(|obj| obj.contains_key("MyApp::Users"))
        .unwrap_or(false);
...
async fn test_schema_validates_attribute_types() {
    ...
    let result = data_store
        .update_entities(valid_entity, schema_store.get_cedar_schema().await)
        .await;
    assert!(result.is_ok());
    ...
    let result = data_store
        .update_entities(invalid_entity, schema_store.get_cedar_schema().await)
        .await;
    assert!(result.is_err());
}
...
async fn test_add_action_to_schema() {
    ...
    let schema = schema_store.get_internal_schema().await;
    let actions = schema.get().get("").and_then(|ns| ns.get("actions"));
