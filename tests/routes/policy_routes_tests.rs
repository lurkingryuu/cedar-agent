...
async fn test_update_single_policy() {
    ...
    let result = policy_store.update_policy("update_me".to_string(), update, None).await;
    assert!(result.is_ok());
...
async fn test_update_policy_with_invalid_content() {
    ...
    let result = policy_store.update_policy("will_fail".to_string(), update, None).await;
    assert!(result.is_err());
}
...
async fn test_policy_validation_with_schema() {
    ...
    let result = policy_store.create_policy(&valid_policy, schema_store.get_cedar_schema().await).await;
    assert!(result.is_ok());
}
...
async fn test_policy_invalid_after_schema_change() {
    ...
    let result = policy_store.update_policies(policies, schema_store.get_cedar_schema().await).await;
...
async fn test_create_policy_invalid_for_schema() {
    ...
    let result = policy_store.create_policy(&invalid_policy, schema_store.get_cedar_schema().await).await;
    assert!(result.is_err());
    ...
    assert!(err_msg.contains("validat") || err_msg.contains("NonExistent") || err_msg.contains("unrecognized"));
}
...
async fn test_get_all_policies() {
    ...
    // Add some policies
    policy_store.create_policy(&sample_policy("p1"), None).await.unwrap();
    policy_store.create_policy(&sample_policy("p2"), None).await.unwrap();
    policy_store.create_policy(&sample_policy("p3"), None).await.unwrap();
...
async fn test_policy_set_consistency() {
    ...
    use std::str::FromStr;
    assert!(policy_set.policy(&cedar_policy::PolicyId::from_str("p1").unwrap()).is_some());
    assert!(policy_set.policy(&cedar_policy::PolicyId::from_str("p2").unwrap()).is_some());
    ...
    assert!(policy_set.policy(&cedar_policy::PolicyId::from_str("p1").unwrap()).is_none());
    assert!(policy_set.policy(&cedar_policy::PolicyId::from_str("p2").unwrap()).is_some());
}
